...
import smtplib
import os
from uoapp.models import UserSession
from uoapp.models import User_Groups,User_Login_Details,User_Details
import time
import base64
import urllib
import http.client
	...
	def post(self,request):
		...
		try:
			with connection.cursor() as cursor:
				original = cursor.execute("SELECT password FROM uoapp_user_login_details WHERE email=%s",(email,))
	...
	def post(self,request):
		...
		try:
			...
			with transaction.atomic():	
				#cursor.execute("INSERT INTO uoapp_user_login_details(email,password) VALUES (%s,%s)",(email,password))
				#cursor.execute("INSERT INTO uoapp_user_details(email,username,mobile) VALUES (%s,%s,%s)",(email,username,mobile))
...
class MainPageView(TemplateView):
	def get(self,request):
		...
		if len(user_sessions) == 0:
			...
			with connection.cursor() as cursor:
				#cursor.execute("SELECT DISTINCT groupname FROM uoapp_user_groups ")
				#print (cursor.fetchall())
				#groups = json.loads(json.dumps(cursor.fetchall()))
				#print (groups)

				cursor.execute("SELECT id,incident_number,region,status,incident_opened_date FROM uoapp_email_history ORDER BY id DESC LIMIT 10")
				...
				cursor.execute("SELECT id,message FROM uoapp_message_history ORDER BY id DESC LIMIT 10")
				rows = json.loads(json.dumps(cursor.fetchall()))
...
<th scope="col" colspan="2">Header</th>
		...
		with connection.cursor() as cursor:
			cursor.execute("SELECT email FROM uoapp_user_groups WHERE groupname=%s",(group,))	
...
class MsgPageView(TemplateView):  
	def post(self,request):
		...
		try:	
			with connection.cursor() as cursor:
				cursor.execute("SELECT D.mobile FROM uoapp_user_details D inner join uoapp_user_groups G WHERE G.groupname=%s",(group,))	
				...
				cursor.execute("INSERT INTO uoapp_message_history(groupname,message) VALUES(%s,%s)",(group,data)) 
...
class UsersPageView(TemplateView):  
	def post(self,request):
		...
		with connection.cursor() as cursor:
			cursor.execute("SELECT username FROM uoapp_user_details WHERE groupname=%s",(data,))	
...
class EmailData(TemplateView):  
	def post(self,request):
		...
		with connection.cursor() as cursor:
			cursor.execute("SELECT * FROM uoapp_email_history WHERE id=%s",(id1,))	
...
class MsgData(TemplateView):  
	def post(self,request):
		...
		with connection.cursor() as cursor:
			cursor.execute("SELECT * FROM uoapp_msg_history WHERE id=%s",(id1,))	
			data = cursor.fetchall()
			data = list(data[0])
			data.pop(0)
			group = data.pop()
...
class CallData(TemplateView):  
	def post(self,request):
		...
		with connection.cursor() as cursor:
			cursor.execute("SELECT * FROM uoapp_call_history WHERE id=%s",(id1,))	
			data = cursor.fetchall()
			data = list(data[0])
			data.pop(0)
			group = data.pop()
...
class EmailLoad(TemplateView):
	def post(self,request):
		...
		with connection.cursor() as cursor:
			cursor.execute("SELECT id,what_happened FROM uoapp_email_history WHERE id>%s ORDER BY id DESC LIMIT 10",(lim,))
			rows = json.loads(json.dumps(cursor.fetchall()))
...
class MsgLoad(TemplateView):
	def post(self,request):
		...
		with connection.cursor() as cursor:
			cursor.execute("SELECT id,message FROM uoapp_message_history WHERE id>%s ORDER BY id DESC LIMIT 10",(lim,))
			rows = json.loads(json.dumps(cursor.fetchall()))
...
class CallLoad(TemplateView):
	def post(self,request):
		...
		with connection.cursor() as cursor:
			cursor.execute("SELECT id,message FROM uoapp_call_history WHERE id>%s ORDER BY id DESC LIMIT 10",(lim,))
			rows = json.loads(json.dumps(cursor.fetchall()))
